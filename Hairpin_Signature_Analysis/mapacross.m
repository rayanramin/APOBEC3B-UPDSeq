...
if isnumeric(B) && size(B,1)>1 && size(B,2)>1, nb=size(B,1); else nb=length(B); end   % 2021-07-15 MSL --> changed to fix 
if isnumeric(C) && size(C,1)>1 && size(C,2)>1, nc=size(C,1); else nc=length(C); end   % changed to fix edge case when C is wider than long, e.g. [2x3] matrix of two RGB colors
if nb ~= nc, error('length(B) ~= length(C)'); end                                    
