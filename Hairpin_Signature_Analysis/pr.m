...
if ~isempty(v)
  ...
  if iscellstr(targ)
    for fi=1:length(x)
      if iscellstr(x{fi}) || isstring(x{fi})
        if length(targ)==1
...
if ~isempty(v)
  if length(v)==1 && (ischar(v{1}) || isstring(v{1}))
  ...
  for c=1:ncol
    if i==0
      ...
      if size(x{c},1)>1 && size(x{c},2)>1
        ...
        elseif iscell(x{c}) || isstring(x{c})
          if isstring(x{c}) && ismissing(x{c}(i))
            val = '---';
          else
            val = x{c}{i};
          end
          if (ischar(val) || isstring(val)) && size(val,1)==1  % strings
