...
n_header_lines = 1 + n_comment_lines;

% auto-detect TSV vs. CSV (added 2021-05-10)

z = get_first_lines(filename,n_header_lines);
z = z{end};
filename_csv = strcmpi('csv',filename(end-2:end));
filename_tsv = strcmpi('tsv',filename(end-2:end));
header_csv = contains(z,',');
header_tsv = contains(z,char(9));

if filename_csv && header_tsv
  fprintf('Warning: appears to be TSV not CSV.\n');
  separator = char(9);
elseif filename_tsv && header_csv
  fprintf('Warning: appears to be CSV not TSV.\n');
  separator = ',';
else
  if filename_csv || header_csv && ~header_tsv
    separator = ',';
  elseif filename_tsv || header_tsv
    separator = char(9);
  else
    % last resort = pre-2021 default: assume tabs
    separator = char(9);
  end
end

% autodetect missing first column name (R convention)   [added 2021-05-10]
if n_header_lines==1
  try  % (will fail for "empty" files)
    z = get_first_lines(filename,2);
    if sum(z{1}==separator)+1 == sum(z{2}==separator)
      fprintf('Warning: imputing missing header for first column.\n');
      n_header_lines = -1;        % (understood by Gaddy's read_table)
      R_conversion_flag = true;   % (for converting Gaddy's "EMPTY" to "name" if possible)
    end    
  catch me
  end
end
...
if length(args)==2, args = [args {separator}]; end         % separator character
if length(args)==3, args = [args {n_header_lines}]; end    % number of header lines
...
if args{4} == 0 && n_header_lines > 1
  args{4} = n_header_lines-1; 
...
% LOAD TABLE
try
  table = read_table(args{:});     % Gaddy Getz pre-2007
  nf = length(table.dat);
  if exist('R_conversion_flag','var') && strcmp('EMPTY',table.headers{1}{1}) && ~ismember('name',table.headers{1}(2:end))
    table.headers{1}{1}='name';   % added 2021-05-11
  end
